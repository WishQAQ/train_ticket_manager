          ...
          <el-table-column
            ...
            <template slot-scope="scope">
              <el-dropdown trigger="click">
                <el-button size="mini" :disabled="scope.row.role_name === '超级管理员' || scope.row.role_name === '普通用户'">操作</el-button>
      ...
      <el-form class="tree_form" ref="form" label-width="120px">
        <el-form-item label="角色名称">
          <el-input
              maxlength="10"
              show-word-limit
              :disabled="userInfo.role_name === '超级管理员' || userInfo.role_name === '普通用户'"
          ...
          <div class="role_box">
            <div class="role_list" v-for="item in roleList" :key="item.id">
              ...
              <div class="role_setting">
                ...
                <el-switch
                    v-if="item.name !== '批量编辑' && item.name !== '批量对账' && item.name !== '对账' && item.name !== '锁定' && item.name !== '误餐费'"
                    @input="change($event)"
                    @change="roleSwitch(item)"
